...
from gestion.api.serializers.tipos_serializers import (
    TipoLavadoSerializer, TipoMantenimientoSerializer, TipoCombustibleSerializer, TipoVehiculoSerializer
)
...
class TipoVehiculoViewSet(viewsets.ModelViewSet):
    ...
    def list(self, request):
        tipovehiculo_serializer = self.get_serializer(self.get_queryset(), many = True)
        return Response(tipovehiculo_serializer.data, status = status.HTTP_200_OK)
    
    def create(self,request):
        serializer = self.serializer_class(data = request.data)
        if serializer.is_valid():
            serializer.save()
            return Response({'message': 'Tipo de vehículo creado correctamente!'},status = status.HTTP_201_CREATED)
        return Response(serializer.errors,status = status.HTTP_400_BAD_REQUEST)